...
def get_data_meteo_api(city: str, start_date: str, end_date: str):
    ...
    try:
        ...
        logger.error("Validation error for city=¨%s, start=%s, end=%s: %s", city, start_date, end_date, e)
        return None
...
def get_retry_session():
    """
    Returns a requests.Session with a retry policy.
    - up to 3 attempts
    - with a backoff factor of 0.5 (waits 0.5s, 1s, 2s)
    - only for 5xx server errors
    """
...
def call_api(city: str, start_date: str, end_date: str) -> Optional[MeteoResponse]:
    """
    ...
    try:
        ...
        logger.error("Connection error fetching data for city=%s: %s", city, e)
        return None
    except requests.exceptions.Timeout as e:
        logger.error("Request timed out for city=%s: %s", city, e)
        return None

    if response.status_code != 200:
        logger.error("Unexpected status code %s for city=%s", response.status_code, city)
        return None

    try:
        validated = MeteoResponse.model_validate(response.json())
    except ValidationError as e:
        logger.error("Schema validation failed for city=%s: %s", city, e)
        return None
...
def main(cities: list[str], start_date: str, end_date: str):
    """
    ...
    for city in cities:
        data = get_data_meteo_api(city, start_date=start_date, end_date=end_date)
        if data is None:
            continue
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch and plot weather data.")
    parser.add_argument(
        "--cities",
        nargs="+",
        default=list(COORDINATES.keys()),
        help=f"Cities to fetch data for. Knows about: {', '.join(COORDINATES.keys())}",
    )
    parser.add_argument(
        "--start-date",
        type=str,
    ...
    main(cities=args.cities, start_date=args.start_date, end_date=args.end_date)